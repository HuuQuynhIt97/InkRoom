...
namespace INK_API.Controllers
{
    ...
    public class SettingWorkPlanController : ControllerBase
    {
        private readonly ISettingWorkPlanService _service;
        public SettingWorkPlanController(ISettingWorkPlanService Service)
        {
            _service = Service;
        }
        ...
        {
            var lists = await _service.GetLines(buildingID);
            return Ok(lists);
        }

        [HttpGet("{line}")]
        public async Task<IActionResult> GetPoSettingWorkPlan(string line)
        {
            var lists = await _service.GetPoSettingWorkPlan(line);
            return Ok(lists);
        }
