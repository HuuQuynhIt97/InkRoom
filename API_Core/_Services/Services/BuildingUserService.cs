...
namespace INK_API._Services.Services
{
    public class BuildingUserService : IBuildingUserService
    {
        ...
        {
            return await _buildingUserRepository.FindAll(x => x.BuildingID == buildingID).ProjectTo<BuildingUserDto>(_configMapper).ToListAsync();
        }
        ...
        {
            var item = await _buildingUserRepository.FindAll(x => x.UserID == userid).FirstOrDefaultAsync();
            if (item == null)
            {
        ...
        {
            var item =await _buildingUserRepository.FindAll(x => x.UserID == buildingUserDto.UserID && x.BuildingID == buildingUserDto.BuildingID).FirstOrDefaultAsync();
            if (item == null)
            {
                _buildingUserRepository.Add(new BuildingUser { 
